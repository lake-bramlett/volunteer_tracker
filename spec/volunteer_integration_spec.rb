...
describe 'the project update path', {:type => :feature} do
  it 'allows a user to change the name of the project' do
    test_project = Project.new({:title => 'Teaching Kids to Code', :total_hours => 0, :id => nil})
    test_project.save
    visit '/'
...
describe 'the volunteer detail page path', {:type => :feature} do
  it 'shows a volunteer detail page' do
    ...
    test_volunteer = Volunteer.new({:name => 'Jasmine', :project_id => project_id, :hours => 0, :id => nil})
    test_volunteer.save
